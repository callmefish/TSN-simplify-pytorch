...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 dropout=0.5, crop_num=1, partial_bn=True
                 ):
        super().__init__()
    ...
    def _construct_model(self, channels):
        ...
        new_kernel_size = kernel_size[:1] + (channels * self.new_length,) + kernel_size[
                                                                            2:]  # optiecal flow个数*2(x,y两个方向) #RGBDiff 3个通道
    ...
    def _get_diff(self, input):
        ...
        input_view = input.view(
            (-1, self.num_segments, self.new_length + 1, input_c,) + input.size()[2:])  # (batch,seg,len,3,h,w)
